...
//x:0~127
//y:0~63
void OLED_DrawPoint2(u8 x,u8 y)
{
	u8 i,m,n;
	i=y/8;
	m=y%8;
...
//x:0~127
//y:0~63
void OLED_ClearPoint2(u8 x,u8 y)
{
	u8 i,m,n;
	i=y/8;
	m=y%8;
...
void OLED_DrawLine(u8 x1,u8 y1,u8 x2,u8 y2)
{
	...
	else      //画斜线
	{
		...
		for(i=0;i<(x2-x1);i++)
			{
			  OLED_DrawPoint2(x1+i,y1+i*k/10);
			}
	}
}
...
void OLED_ClearLine(u8 x1,u8 y1,u8 x2,u8 y2)
{
	...
	else      //画斜线
	{
		...
		for(i=0;i<(x2-x1);i++)
			{
			  OLED_ClearPoint2(x1+i,y1+i*k/10);
			}
	}
}
